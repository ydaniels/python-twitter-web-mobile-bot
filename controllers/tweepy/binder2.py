...
def me(poxi):
   global call
   if call==1:
	#print('calling me secon time i broke')
	return 
   else:
	global proxy
	proxy=poxi
	#print ('first time',poxi,proxy)
	call=call+1
def bind_api(**config):
    api = config['api']
    global poxi
	...
	api = config['api']
        ...
        def execute(self):
		    ...
		    print(proxy)
                    ...
                    resp = self.session.request(str(self.method),
                                                full_url,
                                                data=self.post_data,
                    ...
                    #print('am empty',proxy)
                    #self.re=requests.get('http://google.com',proxies=proxy)
		    #print(dir(self.re))
                    #print( self.re.url)
